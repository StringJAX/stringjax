# Contributing

Contributions to the StringJAX ecosystem are very welcome. Most physics
contributions belong in the relevant **member** repository (JAXVacua, JAXPolyLog,
StringForge); this `stringjax` repository hosts only the metapackage, the
`stringjax` command, and this documentation hub.

For the full guidelines — development setup, running the tests, building the docs,
and the pull-request checklist — see the
[`CONTRIBUTING.md`](https://github.com/StringJAX/stringjax/blob/main/CONTRIBUTING.md)
in the repository root.

Questions and "how do I …?" discussions are best raised in GitHub Discussions.
